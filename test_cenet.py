...
def test_ce_net_ORIGA(root_path, weight_path, dataset):
    ...
    elif test_dataset_category_name == 'tum':
        image_root = '/data/baisr/bsisr/tumor/test/img'#.replace('/data/baisr/', '/vepfs/d_bsr/')
        gt_root = '/data/baisr/bsisr/tumor/test/mask'#.replace('/data/baisr/', '/vepfs/d_bsr/')
        dataid = 2
        images_list = []
        masks_list = []
        for image_name in os.listdir(image_root):
    ...
    if len(os.listdir(target)) == len(masks_list) + 1:
        ...
        for i in range(len(masks_list)):
            ...
            if np.max(ground_truth) != 0:
                iou, dice, auprc = accuracy(new_mask, ground_truth)
                if dice != 0:
...
if __name__=='__main__':
    ...
    weight_path = './weights/CE_Net_NEW11FUSIONidridMABCELOSS.th'
    test_ce_net_ORIGA(root_path, weight_path, 'tum')