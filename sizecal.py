...
def read_tumor(root_path= '', mode='train'):
    images = []
    masks = []

    if mode == 'train':
        image_root = '/data/baisr/bsisr/tumor/JPEGImages'
        gt_root = '/data/baisr/bsisr/tumor/segmask_visual'
    else:
    ...
    for label_name in os.listdir(gt_root):
        image_path = os.path.join(image_root, label_name.replace('.png', '.jpg'))
...
if __name__=='__main__':
    ...
    for img_name in mask:
        ...
        if count_255 != 0:
            tumorsize.append(count_255)
    tumorsize = np.sort(np.array(tumorsize))
    tu10 = tumorsize[int(len(tumorsize) * 0.1)]
    tu90 = tumorsize[int(len(tumorsize) * 0.9)]
    print(tu10)
    print(tu90)
    plt.figure()
    ax1 = plt.gca()
